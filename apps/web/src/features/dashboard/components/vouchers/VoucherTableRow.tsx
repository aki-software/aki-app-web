...
import { BadgeCheck, Building2, Ticket, UserRound, Calendar, Copy, Check, Share2, MessageCircle, Mail, Send, X } from "lucide-react";
...
function statusClasses(status: string) {
  switch (status) {
    case "AVAILABLE": return "text-emerald-500 border-emerald-500/20 bg-emerald-500/5 shadow-emerald-500/10";
    case "USED": return "text-app-primary border-app-primary/20 bg-app-primary/5 shadow-app-primary/10";
    case "EXPIRED": return "text-rose-500 border-rose-500/20 bg-rose-500/5 shadow-rose-500/10";
...
export function VoucherTableRow({ voucher, isAdmin }: Props) {
  const [copiedType, setCopiedType] = useState<'CODE' | 'LINK' | 'MAIL' | null>(null);
  ...
  const handleWhatsApp = () => {
    const message = encodeURIComponent(`Hola! Este es tu código para realizar el test vocacional en A.kit: ${voucher.code}\n\nPodés ingresar directamente aquí: ${testUrl}`);
        ...
        <td className="px-5 py-4">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-1.5 text-[10px] font-black text-app-text-main uppercase tracking-tight whitespace-nowrap">
              ...
              {voucher.ownerInstitutionName || "Venta Particular"}
            </div>
        ...
        <div className="flex items-center justify-end gap-1.5 relative">
          {voucher.status === "AVAILABLE" ? (
            <>
                ...
                <div className="absolute right-0 -top-20 z-[100] animate-in fade-in zoom-in slide-in-from-bottom-4 duration-300">
                    <div className="app-card !p-3 flex flex-col gap-3 shadow-2xl border-app-primary bg-app-surface min-w-[320px] ring-4 ring-black/5">
                        <div className="flex items-center justify-between px-2 pt-1">
                            ...
                            <button onClick={() => setShowEmailInput(false)} className="text-app-text-muted hover:text-rose-500 transition-colors">
                                <X className="h-3 w-3" />
                            </button>
                        </div>
                        <div className="flex items-center gap-3 p-1 bg-app-bg rounded-xl border border-app-border group-focus-within:border-app-primary transition-all">
                            ...
                            >
                                {sendingEmail ? <div className="h-4 w-4 border-2 border-white/50 border-t-white animate-spin rounded-full" /> : <Send className="h-4 w-4" />}
                            </button>
                        </div>
                    </div>
                </div>
              ...
              >
                {copiedType === 'LINK' ? <Check className="h-3.5 w-3.5" /> : <Share2 className="h-3.5 w-3.5" />}
              </button>
              <button 
            ...
            <div className="app-tag !px-3 !py-1 opacity-20 !bg-transparent border-dashed">Consumido</div>
          )}
        </div>
      </td>
    </tr>
