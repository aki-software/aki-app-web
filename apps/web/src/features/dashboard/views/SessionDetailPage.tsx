  ...
  if (loading || categoriesLoading) {
    return (
      <div className="flex h-96 flex-col items-center justify-center gap-4 text-app-text-muted">
        ...
        <span className="app-label mt-4 opacity-50">Generando Analítica Lux 3.0</span>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-10">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-4">
                ...
                <span className="app-label">INFORME TÉCNICO CERTIFICADO</span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
        ...
        <div className="lg:col-span-12 xl:col-span-12 space-y-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
              ...
              <div className="app-card shadow-2xl relative overflow-hidden group">
                <div className="flex items-center justify-between mb-12">
                    <div>
                        <h3 className="app-value !text-2xl mt-0">Cruce de Variables</h3>
                        <p className="app-label mt-3 opacity-60">ANÁLISIS RIASEC DE 12 PUNTOS</p>
                    </div>
                ...
                <div className="mt-12 grid grid-cols-2 gap-8 border-t border-app-border pt-12">
                    <div className="flex flex-col items-center text-center">
                        ...
                        <span className="text-sm font-black text-app-text-main uppercase tracking-widest">{formatDate(session.sessionDate)}</span>
                    </div>
                    <div className="flex flex-col items-center text-center">
                        <span className="app-label mb-3 opacity-40">TIEMPO EJECUCIÓN</span>
                        <span className="text-sm font-black text-app-text-main uppercase tracking-widest">{formatTime(session.totalTimeMs)}</span>
                    </div>
                </div>
              </div>
              ...
              <div className="app-card shadow-2xl border-l-[8px] !border-l-emerald-500 flex flex-col justify-between">
                <div>
                   <div className="flex items-center gap-6 mb-12">
                        ...
                        <div>
                            <h4 className="app-value !text-2xl mt-0">Consistencia Ética</h4>
                            <p className="app-label mt-2">Nivel de veracidad: <b className="text-emerald-500">{behaviorStats?.reliabilityLevel}</b></p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-12 py-4">
                        <div className="flex flex-col items-center">
                            <div className="flex justify-between items-center w-full mb-4">
                                <span className="app-label opacity-60">INDICADOR DE DUDA</span>
                                <span className="app-tag !text-[11px] !px-4 !py-1 transition-all group-hover:scale-105 active:scale-95">{behaviorStats?.undosCount} retrocesos</span>
                            </div>
                            <div className="h-3 w-full bg-app-bg border border-app-border rounded-full overflow-hidden p-0.5">
                                <div className="h-full bg-emerald-500 rounded-full transition-all duration-1000 shadow-[0_0_15px_rgba(16,185,129,0.3)]" style={{ width: `${Math.max(5, 100 - (behaviorStats?.undosCount || 0) * 8)}%` }}></div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col items-center">
                            <div className="flex justify-between items-center w-full mb-4">
                                ...
                                <span className="app-tag !text-[11px] !px-4 !py-1">{behaviorStats?.avgTime ? formatTime(behaviorStats.avgTime) : "—"}</span>
                            </div>
                            <div className="w-full flex items-center justify-center gap-3 p-6 rounded-[1.5rem] bg-app-bg border border-app-border border-dashed transition-all hover:bg-emerald-500/5 hover:border-emerald-500/20">
                                ...
                                <span className="text-[10px] font-black text-app-text-muted uppercase tracking-[0.2em] text-center">Ritmo técnico sostenido detectado en la toma.</span>
                            </div>
                        </div>
                    </div>
                </div>
          ...
          <div className="grid grid-cols-1 gap-12">
            <SessionTopAreas top3={top3} bottom3={bottom3} categoriesMap={categoriesMap} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              <div className="app-card !p-8 shadow-xl hover:shadow-2xl transition-all group">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <span className="app-label opacity-40">ORIGEN DEL TEST</span>
                        <p className="app-value !text-lg mt-1 font-black">{session.paymentStatus === "PAID" ? "VENTA DIRECTA" : "CANJE VOUCHER"}</p>
                    </div>
                </div>
              </div>
              ...
              <div className="app-card !p-8 shadow-xl hover:shadow-2xl transition-all group">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <span className="app-label opacity-40">SUPERVISIÓN TÉCNICA</span>
                        <p className="app-value !text-lg mt-1 font-black leading-tight uppercase">{session.institutionName || "PARTICULAR"}</p>
                    </div>
                </div>
              </div>
            </div>
                ...
                >
                    <div className="flex items-center gap-6">
                        ...
                        <div className="flex flex-col items-start gap-1">
                            <span className="app-value !text-xl mt-0">Métricas de Competencia</span>
                            <span className="app-label opacity-40 uppercase tracking-[0.4em]">Análisis Desglosado por Áreas</span>
                        </div>
                    </div>
                    <div className="h-16 w-16 flex items-center justify-center rounded-full bg-app-bg border border-app-border group-hover:bg-app-primary group-hover:text-white transition-all">
                        {showTechnicalData ? <ChevronUp className="h-6 w-6" /> : <ChevronDown className="h-6 w-6" />}
                    </div>
                </button>
                
                {showTechnicalData && (
                    <div className="mt-12 animate-in duration-700">
                        <SessionCategoryChart sortedResults={sortedResults} top3={top3} categoriesMap={categoriesMap} />
                    </div>
                )}
            </div>
          </div>
