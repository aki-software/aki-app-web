...
export function PlanCard({
  ...
  return (
    <div
      className={`group relative w-full h-full flex flex-col transition-all duration-300 rounded-3xl border-2 overflow-hidden bg-app-surface ${
        isSelected
    ...
    >
      ...
      <div
        className={`absolute top-0 inset-x-0 h-32 bg-gradient-to-b opacity-10 pointer-events-none ${
          isSelected
            ? "from-app-primary to-transparent"
            : "from-app-text-main to-transparent"
        }`}
      />
      ...
      <div className="relative z-10 p-7 sm:p-8 flex flex-col flex-1">
        ...
        <div className="flex items-start justify-between gap-2 mb-6">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2">
              <Ticket
                className={`w-4 h-4 ${isSelected ? "text-app-primary" : "text-app-text-muted"}`}
              />
              <span
                className={`text-xs font-bold uppercase tracking-wider ${isSelected ? "text-app-primary" : "text-app-text-muted"}`}
              >
        ...
        <div className="flex items-baseline gap-1 mb-6">
          ...
          <span className="text-sm font-semibold text-app-text-muted uppercase tracking-wider">
            USD
          </span>
        </div>
          ...
          <ul className="space-y-3.5 mb-8 flex-1">
            {descriptionLines.map((line, idx) => (
              <li key={idx} className="flex items-start gap-3">
                ...
                <span className="text-sm text-app-text-main font-medium leading-relaxed">
                  {line.trim()}
                </span>
              </li>
            ))}
        ...
        <div className="mt-auto pt-4">
          ...
          >
            ...
            <span className="relative z-10">
              {actionLabel || (isSelected ? "Seleccionado" : "Seleccionar")}
            </span>
          </button>
        </div>
      </div>
