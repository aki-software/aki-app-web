...
const ICON_MAP: Record<AdminActivityEvent["type"], { icon: LucideIcon; colorClass: string }> = {
  VOUCHER_REDEEMED: { icon: Ticket, colorClass: "text-status-success" },
  VOUCHER_ISSUED: { icon: Ticket, colorClass: "text-app-primary" },
  SESSION_COMPLETED: { icon: ClipboardList, colorClass: "text-status-success" },
  SESSION_STARTED: { icon: Activity, colorClass: "text-status-warning" },
};
...
export function EventIcon({ type, className = "h-4 w-4" }: EventIconProps) {
  ...
  return <Icon className={`${className} ${config.colorClass}`} aria-hidden="true" />;
}
