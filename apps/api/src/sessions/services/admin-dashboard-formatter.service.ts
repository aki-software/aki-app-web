...
@Injectable()
export class AdminDashboardFormatterService {
  getPeriodStart(periodDays: number, now: Date): Date {
  ...
  buildOverviewPayload(input: {
    ...
    const totalSessions = this.parseIntSafe(input.sessionTotals.totalSessions);
    const completedSessions = this.parseIntSafe(
      input.sessionTotals.completedSessions,
    );
    ...
    const totalTimeMs = this.parseBigNumberSafe(
      input.sessionTotals.totalTimeMs,
    );
    const averageTimeSeconds =
      totalSessions > 0 ? Math.floor(totalTimeMs / totalSessions / 1000) : 0;

    const vouchersGeneratedPeriod = this.parseIntSafe(
      input.periodVoucherStats.issued,
    );
    const vouchersRedeemedPeriod = this.parseIntSafe(
    ...
    const periodBasicStats = {
      started: this.parseIntSafe(input.periodSessionStats.started),
      completed: this.parseIntSafe(input.periodSessionStats.completed),
      reportsUnlocked: this.parseIntSafe(
        input.periodSessionStats.reportsUnlocked,
      ),
    };
    const periodVoucherStats = {
      started: this.parseIntSafe(input.periodSessionStats.voucherStarted),
      completed: this.parseIntSafe(input.periodSessionStats.voucherCompleted),
      reportsUnlocked: this.parseIntSafe(
        input.periodSessionStats.voucherReportsUnlocked,
      ),
    };
    const individualCompletedCount = this.parseIntSafe(
      input.periodSessionStats.individualCompleted,
    );

    const sessionsActivity = this.formatDailyActivity(
    ...
    return {
      totalSessions,
      totalHistoricalVouchers: this.parseIntSafe(
        input.voucherTotals.historical,
      ),
      completionRate,
      averageTimeSeconds,
      availableVouchers: this.parseIntSafe(input.voucherTotals.available),
      redeemedVouchers: this.parseIntSafe(input.voucherTotals.redeemed),
      ...
      channelBreakdown: {
        ...
        individual: {
          ...
          reportsUnlocked:
            ...
            periodVoucherStats.reportsUnlocked,
        },
      },
      sessionsActivity,
  ...
  formatSessionActivity(rows: RawRecentSessionRow[]): AdminActivityItem[] {
  ...
  private formatDailyActivity(rows: RawSessionsActivityRow[], now: Date) {
