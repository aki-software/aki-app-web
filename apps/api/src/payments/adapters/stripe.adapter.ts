...
export class StripeAdapter implements PaymentGatewayAdapter {
  ...
  constructor() {
    this.stripe = new Stripe(process.env.STRIPE_SECRET_KEY || 'test', { apiVersion: '2024-04-10' as any });
  }
  ...
  async validateWebhook(rawBody: string, headers: Record<string, string>): Promise<boolean> {
    try {
      this.stripe.webhooks.constructEvent(rawBody, headers['stripe-signature'], process.env.STRIPE_WEBHOOK_SECRET || 'test');
      return true;
    } catch {
      return false;
    }
