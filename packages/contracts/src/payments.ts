...
export const PaymentState = z.enum(['PENDING', 'PAID', 'FAILED', 'EXPIRED', 'CANCELLED', 'REFUNDED', 'UNKNOWN']);
export type PaymentState = z.infer<typeof PaymentState>;

export const FulfillmentState = z.enum(['NOT_APPLICABLE', 'QUEUED', 'IN_PROGRESS', 'DELAYED', 'FULFILLED', 'REVOKED', 'BLOCKED']);
export type FulfillmentState = z.infer<typeof FulfillmentState>;

export const ProviderFreshness = z.enum(['CURRENT', 'STALE', 'UNAVAILABLE', 'NOT_OBSERVED']);
...
export const PaymentStatus = z.discriminatedUnion('providerFreshness', [
  z.object({ ...paymentStatusFields, providerFreshness: z.literal('CURRENT'), observedAt: datetime, staleAfter: datetime }).strict(),
  z.object({ ...paymentStatusFields, providerFreshness: z.literal('STALE'), observedAt: datetime, staleAfter: datetime }).strict(),
  z.object({ ...paymentStatusFields, providerFreshness: z.literal('UNAVAILABLE'), observedAt: z.null(), staleAfter: z.null() }).strict(),
  z.object({ ...paymentStatusFields, providerFreshness: z.literal('NOT_OBSERVED'), observedAt: z.null(), staleAfter: z.null() }).strict(),
]);
export type PaymentStatus = z.infer<typeof PaymentStatus>;

export const PaymentHistory = z.object({
...
const sanitizedReference = z.string().trim().min(1).max(256).regex(/^[A-Za-z0-9][A-Za-z0-9._-]*$/);
const remediationAction = z.enum(['CONFIRM_EXTERNAL_CANCELLATION', 'CONFIRM_EXTERNAL_FULL_REFUND_AND_REVOKE']);
...
export const RemediationRequest = z.object({
  ...
  if (request.providerCheckoutId === null && request.providerPaymentId === null) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['providerCheckoutId'], message: 'providerCheckoutId or providerPaymentId is required' });
  }
  if (request.action === 'CONFIRM_EXTERNAL_FULL_REFUND_AND_REVOKE' && request.providerActionReferences.length === 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['providerActionReferences'], message: 'full refund and revoke requires provider action references' });
  }
});
export type RemediationRequest = z.infer<typeof RemediationRequest>;

export const RemediationResponse = z.object({
  decision: z.enum(['ACCEPTED', 'REJECTED', 'DEFERRED', 'CONFLICT', 'DUPLICATE']),
  remediationId: uuid,
  auditId: uuid,
}).strict();
